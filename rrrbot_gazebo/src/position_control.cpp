...
//The below code just takes theta values into another message and publishes them for the main controller to use
class Processor : public rclcpp::Node
{
  public:
  Processor(): Node("pos_control")
  {
    subscription_ = this->create_subscription<sensor_msgs::msg::JointState>("/joint_states", 10, std::bind(&Processor::topic_callback, this, _1)); //subscribing to /joint_states
